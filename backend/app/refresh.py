...
def main():
    init_db()
    _run("[ 1/14] confsearch.ethz.ch",       confsearch.ingest_all)
    _run("[ 2/14] noise-lab",                noise_lab.ingest_all)
    _run("[ 3/14] klb2/conference-calendar", klb2.ingest_all)
    _run("[ 4/14] ds-deadlines",             ds_deadlines.ingest_all)
    _run("[ 5/14] aideadlines",              aideadlines.ingest_all)
    _run("[ 6/14] ccfddl",                   ccfddl.ingest_all)
    _run("[ 7/14] seed YAML",                seed.ingest_seed)
    _run("[ 8/14] user_added YAML",          user_venues.ingest_user_added)
    _run("[ 9/14] cleanup old years",        cleanup.cleanup_old_years)
    _run("[10/14] stats overlay",            seed.apply_stats)
    _run("[11/14] LLM enrichment",           llm_extract.enrich_seed_venues)
    _run("[12/14] reconcile",                reconcile.reconcile)
    _run("[13/14] predict next-year",        predict.predict_next_year)
    _run("[14/14] predict missing tiers",    tier_predict.predict_tiers)
