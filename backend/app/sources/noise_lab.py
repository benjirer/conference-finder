...
ROUND_RX = re.compile(r'Deadline\s*\(\s*(\d+)\s*/\s*(\d+)\s*\)', re.IGNORECASE)
...
def ingest_all() -> dict[str, int]:
    ...
    # by all three so multi-round venues (e.g. NSDI with two cycles) become
    # multiple rows in our DB.
    ...
    for i, anchor in enumerate(anchors):
        ...
        round_m = ROUND_RX.search(block)
        round_idx = int(round_m.group(1)) if round_m else 1
        rounds_total = int(round_m.group(2)) if round_m else None
        ...
        seen[(acronym, year, round_idx)] = {
            "acronym": acronym, "year": year, "round": round_idx,
            "rounds_total": rounds_total,
            "name": full_name or acronym, "link": link, "location": location,
    ...
    with SessionLocal() as db:
        for v in seen.values():
            _common.upsert_source_record(
                db,
                acronym=v["acronym"], year=v["year"], source=SOURCE_NAME, round=v["round"],
            ...
            if _common.upsert_conference_secondary(
                ...
                round=v["round"], rounds_total=v["rounds_total"],
                areas=["networking"],
                submission_deadline=v["submission_deadline"],
                location=v["location"],
