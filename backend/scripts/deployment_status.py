...
with httpx.Client(base_url=f'https://api.github.com/repos/{repo}/', headers={
 ...
 for path in ['actions/permissions/workflow', 'actions/secrets/public-key', 'commits/main/status', 'deployments?per_page=3', 'actions/workflows', 'actions/runs?branch=codex%2Fvenue-reliability&per_page=3']:
  ...
  if path.startswith('actions/runs?') and response.is_success:
   data=[{key:item.get(key) for key in ['name','status','conclusion','html_url','head_sha']} for item in data.get('workflow_runs',[])]
